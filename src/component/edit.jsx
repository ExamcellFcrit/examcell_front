...
export class edit extends Component {
    ...
    add = (dataItem) => {
        ...
        console.log(dataItem)
        /* dataItem.code = this.generateId(sampleProducts); */
        this.setState({
                                    ...
                                    .then(() => {
                                        fetch(`http://192.168.29.101:8000/scheme/${scheme}/branch/${branch}${scheme}/semester/${semester}${branch}${scheme}/course/`, {
                                            ...
                                            .then(
                                                ...
                                                { success() }
                                                /* const failed = () => toast.update(toastId, { type: toast.TYPE.ERROR, autoClose: 2000, render: `${dataItem.course} is already added!` });
                                                { failed() } */
                                            })
                                    })
    ...
    checkTime = (et, st, course, dataItem) => {
        if ((et || st) === null) {
            const success = () => toast.update(toastId, { type: toast.TYPE.SUCCESS, autoClose: 3000, render: `${course} Updates` });
            { success() }
            return
        }
        if (!/^([0-2]?[0-9]|2[0-3]):[0-5][0-9]$/.test(et) || !/^([0-2]?[0-9]|2[0-3]):[0-5][0-9]$/.test(st)) {
            const success = () => toast.update(toastId, { type: toast.TYPE.ERROR, autoClose: 3000, render: `Wrong Format` });
            { success() }
            { this.cancel(dataItem) }
            return
        }
        const success = () => toast.update(toastId, { type: toast.TYPE.SUCCESS, autoClose: 3000, render: `${course} Updated` });
        { success() }
    ...
    cancel = (dataItem) => {
        const originalItem = this.state.initial.find(p => p.code === dataItem.code);
